...
interface EducationProps {
  titleStyle: any;
  schoolStyle: any;
  data: { school: string; degree: string; date: string }[];
  lineStyle: any;
  strokeWidth: number;
  strokeLength: string;
  fontSize: string;
...
const Education: React.FC<EducationProps> = (props) => {
  return (
    <View style={{ marginBottom: 15, fontSize: `${props.fontSize}`, lineHeight: '1.6pt' }}>
      ...
      {props.data.map(({ school, degree, date }, index) => (
        <View key={index} style={{ display: 'flex', flexDirection: 'row' }}>
          <View style={{ display: 'flex', flexDirection: 'row' }}>
            <Text style={props.schoolStyle}>{`${school}, `}</Text>
            <Text>{degree}</Text>
          </View>
          <Text style={{margin: '0 0 0 auto'}}>{date}</Text>
        </View>
      ))}
    </View>
  );
